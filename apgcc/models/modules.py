...
class ifi_simfpn(nn.Module):
    def __init__(self, ultra_pe=False, pos_dim=40, sync_bn=False, num_anchor_points=4, num_classes=2, local=False, unfold=False, 
        ...
        confidence_head_list.append([nn.Conv1d(head_layers[-1], self.num_anchor_points*self.num_classes, 1)])
    ...
    def forward(self, x, size, level=0, after_cat=False):
        ...
        if not after_cat:
            if not self.local:
                ...
                if self.ultra_pe:
                    rel_coord = getattr(self, 'pos' + str(level))(rel_coord)
                elif self.learn_pe:
                    rel_coord = getattr(self, 'pos' + str(level))(rel_coord, [1,1,h,w])
...
def ifi_feat(res, size, stride=1, local=False):
    ...
    h , w = size
    coords = (make_coord((h, w), device=res.device).flip(-1) + 1) / 2
    coords = coords.unsqueeze(0).expand(bs, *coords.shape)
    coords = (coords*2-1).flip(-1)

    feat_coords = make_coord((hh, ww), flatten=False, device=res.device).permute(2, 0, 1) \
        .unsqueeze(0).expand(res.shape[0], 2, *(hh,ww))
